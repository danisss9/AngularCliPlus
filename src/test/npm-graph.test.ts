...
suite('npm dependency graph collection', () => {
  let folder: string;
  setup(async () => {
    folder = await fs.mkdtemp(path.join(os.tmpdir(), 'angular-cli-plus-graph-'));
    await fs.writeFile(path.join(folder, 'package.json'), JSON.stringify({ name: 'app', dependencies: { a: '^1' }, devDependencies: { dev: '~2' } }));
  });
  teardown(async () => {
    assert.ok(path.resolve(folder).startsWith(path.resolve(os.tmpdir()) + path.sep + 'angular-cli-plus-graph-'));
...
suite('npm dependency graph extension integration', () => {
  ...
  test('loads real webviews, reuses panels per folder, and recreates closed panels', async function () {
    ...
    const folder = await fs.mkdtemp(path.join(os.tmpdir(), 'angular-cli-plus-graph-panels-'));
    ...
    try {
      ...
      setExtensionContext(contextBefore);
      assert.ok(path.resolve(folder).startsWith(path.resolve(os.tmpdir()) + path.sep + 'angular-cli-plus-graph-panels-'));
      await fs.rm(folder, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 });
    }
  });
  ...
  test('command activates and has a unique shortcut with a macOS override', async () => {
    ...
    assert.ok((await vscode.commands.getCommands(true)).includes('angular-cli-plus.showNpmDependencyGraph'));
    const bindings = extension.packageJSON.contributes.keybindings as { command: string; key: string; mac?: string }[];
    const binding = bindings.find(item => item.command === 'angular-cli-plus.showNpmDependencyGraph');
    assert.strictEqual(binding?.key, 'ctrl+shift+a f');
    assert.strictEqual(binding?.mac, 'cmd+shift+a f');
