...
suite('semverSatisfies', () => {
  ...
  test('"latest" always passes', () => assert.strictEqual(semverSatisfies('1.2.3', 'latest'), true));
  test('whitespace-only range treated as empty', () => assert.strictEqual(semverSatisfies('1.2.3', '   '), true));

  // Non-semver specifiers
  test('git+https: specifier passes', () => assert.strictEqual(semverSatisfies('1.0.0', 'git+https://github.com/foo/bar'), true));
  test('file: specifier passes', () => assert.strictEqual(semverSatisfies('1.0.0', 'file:../local-pkg'), true));
  test('workspace: specifier passes', () => assert.strictEqual(semverSatisfies('1.0.0', 'workspace:^'), true));
  test('https: specifier passes', () => assert.strictEqual(semverSatisfies('1.0.0', 'https://example.com/pkg.tgz'), true));
  test('github: specifier passes', () => assert.strictEqual(semverSatisfies('1.0.0', 'github:owner/repo'), true));

  // Caret (^)
  test('^ patch upgrade satisfies', () => assert.strictEqual(semverSatisfies('1.2.5', '^1.2.3'), true));
  test('^ minor upgrade satisfies', () => assert.strictEqual(semverSatisfies('1.3.0', '^1.2.3'), true));
  test('^ exact match satisfies', () => assert.strictEqual(semverSatisfies('1.2.3', '^1.2.3'), true));
  ...
  test('^ zero major: same minor required', () => assert.strictEqual(semverSatisfies('0.2.5', '^0.2.3'), true));
  test('^ zero major: different minor fails', () => assert.strictEqual(semverSatisfies('0.3.0', '^0.2.3'), false));
  ...
  test('> strictly greater satisfies', () => assert.strictEqual(semverSatisfies('1.2.4', '>1.2.3'), true));
  ...
  test('< strictly lesser satisfies', () => assert.strictEqual(semverSatisfies('1.2.2', '<1.2.3'), true));
  test('< equal fails', () => assert.strictEqual(semverSatisfies('1.2.3', '<1.2.3'), false));
  ...
  test('exact with = prefix passes', () => assert.strictEqual(semverSatisfies('1.2.3', '=1.2.3'), true));

  // Hyphen range
  test('hyphen range — value inside satisfies', () => assert.strictEqual(semverSatisfies('1.5.0', '1.0.0 - 2.0.0'), true));
  test('hyphen range — lower bound satisfies', () => assert.strictEqual(semverSatisfies('1.0.0', '1.0.0 - 2.0.0'), true));
  test('hyphen range — upper bound satisfies', () => assert.strictEqual(semverSatisfies('2.0.0', '1.0.0 - 2.0.0'), true));
  test('hyphen range — outside fails', () => assert.strictEqual(semverSatisfies('3.0.0', '1.0.0 - 2.0.0'), false));

  // OR range (||)
  test('|| range — first alternative satisfies', () => assert.strictEqual(semverSatisfies('1.2.3', '^1.0.0 || ^2.0.0'), true));
  test('|| range — second alternative satisfies', () => assert.strictEqual(semverSatisfies('2.5.0', '^1.0.0 || ^2.0.0'), true));
  test('|| range — neither alternative fails', () => assert.strictEqual(semverSatisfies('3.0.0', '^1.0.0 || ^2.0.0'), false));

  // Pre-release versions (broken in the old custom parser)
  test('pre-release installed coerces to release for range check', () => assert.strictEqual(semverSatisfies('1.0.0-beta', '^1.0.0'), true));
  test('pre-release installed fails lower range after coercion', () => assert.strictEqual(semverSatisfies('0.9.0-rc1', '^1.0.0'), false));

  // v-prefix
  test('v-prefixed version satisfies range', () => assert.strictEqual(semverSatisfies('v14.21.0', '>=14'), true));
  test('v-prefixed version fails range', () => assert.strictEqual(semverSatisfies('v12.0.0', '>=14'), false));
  test('v-prefixed version satisfies caret', () => assert.strictEqual(semverSatisfies('v18.19.0', '^18.0.0'), true));

  // Unparseable / garbage versions
  test('completely unparseable installed version returns true (safe default)', () => assert.strictEqual(semverSatisfies('not-a-version', '^1.0.0'), true));
});
...
suite('validateCustomCommand', () => {
  // Valid commands
  test('npm install is valid', () => assert.strictEqual(validateCustomCommand('npm install'), null));
  test('pnpm install is valid', () => assert.strictEqual(validateCustomCommand('pnpm install'), null));
  test('yarn --frozen-lockfile is valid', () => assert.strictEqual(validateCustomCommand('yarn install --frozen-lockfile'), null));
  test('bun install is valid', () => assert.strictEqual(validateCustomCommand('bun install'), null));
  test('npm ci is valid', () => assert.strictEqual(validateCustomCommand('npm ci'), null));
  test('command with flags is valid', () => assert.strictEqual(validateCustomCommand('npm install --prefer-offline'), null));

  // Empty / whitespace
  test('empty string is invalid', () => assert.notStrictEqual(validateCustomCommand(''), null));
  test('whitespace-only is invalid', () => assert.notStrictEqual(validateCustomCommand('   '), null));

  // Dangerous injection patterns
  test('; rm injection is blocked', () => assert.notStrictEqual(validateCustomCommand('npm install; rm -rf /'), null));
  test('; del injection is blocked', () => assert.notStrictEqual(validateCustomCommand('npm install; del /F /S /Q C:\\'), null));
  test('; format injection is blocked', () => assert.notStrictEqual(validateCustomCommand('npm install; format C:'), null));
  test('; mkfs injection is blocked', () => assert.notStrictEqual(validateCustomCommand('npm install; mkfs.ext4 /dev/sda'), null));
  test('; dd injection is blocked', () => assert.notStrictEqual(validateCustomCommand('npm install; dd if=/dev/zero of=/dev/sda'), null));
  test('uppercase RM is blocked', () => assert.notStrictEqual(validateCustomCommand('npm install; RM -rf /'), null));

  // Must NOT be over-blocked
  test('"remove" word (not rm) is allowed', () => assert.strictEqual(validateCustomCommand('npm remove lodash'), null));
  test('"install" containing "dd" is allowed', () => assert.strictEqual(validateCustomCommand('npm add lodash'), null));
  test('path containing "del" in name is allowed', () => assert.strictEqual(validateCustomCommand('npm run delete-cache'), null));
...
suite('toKebabCase', () => {
  ...
  test('three words', () => assert.strictEqual(toKebabCase('changeDetectionStrategy'), 'change-detection-strategy'));
  test('already kebab unchanged', () => assert.strictEqual(toKebabCase('skip-tests'), 'skip-tests'));
  test('all lowercase unchanged', () => assert.strictEqual(toKebabCase('standalone'), 'standalone'));
  test('single word unchanged', () => assert.strictEqual(toKebabCase('flat'), 'flat'));
  test('number before capital', () => assert.strictEqual(toKebabCase('form2Builder'), 'form2-builder'));
...
suite('isValidSchematicName', () => {
  // Valid
  test('simple hyphenated name', () => assert.strictEqual(isValidSchematicName('my-component'), true));
  ...
  test('all lowercase letters', () => assert.strictEqual(isValidSchematicName('userprofile'), true));
...
suite('buildNgGenerateCommand', () => {
  test('no options → minimal command', () => assert.strictEqual(buildNgGenerateCommand('component', {}), 'ng generate component'));
  test('boolean true → bare flag', () => {
    const cmd = buildNgGenerateCommand('component', { standalone: true });
    assert.ok(cmd.includes('--standalone'));
    assert.ok(!cmd.includes('--standalone='));
  });
  test('boolean false → flag=false', () => assert.ok(buildNgGenerateCommand('component', { skipTests: false }).includes('--skip-tests=false')));
  test('string value → flag=value', () => assert.ok(buildNgGenerateCommand('component', { style: 'scss' }).includes('--style=scss')));
  test('camelCase key → kebab-case flag', () => assert.ok(buildNgGenerateCommand('component', { changeDetection: 'OnPush' }).includes('--change-detection=OnPush')));
  ...
  test('multiple options all appear', () => {
    const cmd = buildNgGenerateCommand('component', { standalone: true, skipTests: false, style: 'scss' });
    assert.ok(cmd.includes('--standalone'));
  ...
  test('starts with ng generate <type> for all schematic types', () => {
    const types = ['component', 'service', 'module', 'directive', 'pipe',
      'guard', 'interceptor', 'class', 'interface', 'enum', 'resolver'] as const;
...
suite('findMatchingProjects', () => {
  ...
  test('folder inside app/src matches app', () => {
    const matches = findMatchingProjects(W('/workspace/projects/app/src/components'), workspaceRoot, projects);
    assert.ok(matches.includes('app'));
  });
  test('folder inside lib/src matches lib', () => {
    const matches = findMatchingProjects(W('/workspace/projects/lib/src/utils'), workspaceRoot, projects);
  ...
  test('project with absolute root path is handled', () => {
    ...
    const matches = findMatchingProjects(W('/workspace/projects/abs-app/src'), workspaceRoot, absProjects);
...
suite('findBestProjectForPath', () => {
  ...
  test('file inside app/src → app', () => {
    const result = findBestProjectForPath(W('/workspace/projects/app/src/app/app.component.ts'), workspaceRoot, projects);
    assert.strictEqual(result, 'app');
  });
  test('file inside lib/src → lib', () => {
    const result = findBestProjectForPath(W('/workspace/projects/lib/src/lib/lib.ts'), workspaceRoot, projects);
  ...
  test('prefers longer (more specific) root over shorter root', () => {
    ...
    const result = findBestProjectForPath(W('/workspace/projects/app/src/app.ts'), workspaceRoot, projects);
    assert.strictEqual(result, 'app');
  });
  ...
  test('nested project wins over parent project', () => {
    ...
    const result = findBestProjectForPath(W('/workspace/packages/child/src/main.ts'), workspaceRoot, nested);
