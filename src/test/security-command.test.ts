...
suite('Security review extension integration', () => {
  test('registers the command with the expected review defaults', async () => {
    const extension = vscode.extensions.getExtension('danisss9.angular-cli-plus');
    assert.ok(extension);
    await extension.activate();
    assert.ok(
      (await vscode.commands.getCommands(true)).includes('angular-cli-plus.reviewPackageSecurity'),
    );
    const properties = extension.packageJSON.contributes.configuration.properties;
    assert.equal(properties['angularCliPlus.securityReview.afterInstall.enabled'].default, false);
    assert.equal(properties['angularCliPlus.securityReview.npmAudit.enabled'].default, true);
    assert.equal(
      extension.packageJSON.contributes.commands.find(
        (command: { command: string }) =>
          command.command === 'angular-cli-plus.reviewPackageSecurity',
      ).enablement,
      'isWorkspaceTrusted',
    );
  });
  test('normal, forced, clean and custom installs produce one review after their final outcome', async () => {
    ...
    const previous = {
      spawn: managed.spawnManaged,
      begin: security.beginSecurityInstall,
      end: security.endSecurityInstall,
    ...
    const invocations: string[][] = [],
      ...
      ends: unknown[] = [];
    try {
      managed.spawnManaged = async (command, args) => {
      ...
      security.beginSecurityInstall = async (selected) => {
        starts.push(selected);
        return selected;
      };
      security.endSecurityInstall = (selected, outcome) => {
        ends.push({ selected, outcome });
      };
      vscode.workspace.getConfiguration = (() => ({
        get: (key: string) => custom[key],
      ...
      vscode.window.showErrorMessage = (async () =>
        decisions.shift()) as typeof vscode.window.showErrorMessage;
      for (const scenario of [
        ...
        { clean: false, force: true, codes: [1], decisions: [], expected: 'failed', calls: 1 },
        {
          ...
          force: false,
          codes: [1, 1, 0],
          decisions: ['Run Clean Install', 'Run with --force'],
          expected: 'success',
          calls: 3,
        },
        {
          ...
          expected: 'failed',
          calls: 1,
          custom: { 'npm.installCommand': 'pnpm install' },
        },
        {
          clean: true,
          force: false,
          ...
          expected: 'success',
          calls: 1,
          custom: { 'npm.cleanInstallCommand': 'yarn install' },
        },
      ]) {
        starts.length = 0;
        ends.length = 0;
        invocations.length = 0;
        exitCodes = [...scenario.codes];
        decisions = [...scenario.decisions];
        custom = scenario.custom ?? {};
        await runNpmInstall(scenario.clean, scenario.force, root);
        assert.deepEqual(starts, [root]);
        assert.deepEqual(ends, [{ selected: root, outcome: scenario.expected }]);
        assert.equal(invocations.length, scenario.calls);
      }
    } finally {
      managed.spawnManaged = previous.spawn;
      security.beginSecurityInstall = previous.begin;
      security.endSecurityInstall = previous.end;
      vscode.workspace.getConfiguration = previous.config;
      vscode.window.showInformationMessage = previous.info;
      vscode.window.showErrorMessage = previous.error;
      assert.ok(
        contained(os.tmpdir(), root) && path.basename(root).startsWith('acp-security-installs-'),
      );
  ...
  test('file navigation requires a report-owned ID/index and rejects paths or links outside the workspace', async () => {
    ...
    try {
        ...
        findings: [
          {
            id: 'approved',
            evidence: [
              { file: 'install.js', line: 2, column: 1 },
              { file: '../outside', line: 1, column: 1 },
              { file: 'external', line: 1, column: 1 },
            ],
          },
        ],
      } as SecurityReviewReport;
      assert.equal(
        (await resolveEvidenceFile(root, report, 'approved', 0))?.file,
      ...
      assert.ok(
        contained(temporaryRoot, root) &&
          path.basename(root).startsWith('acp-security-navigation-'),
      );
      await fs.rm(root, { recursive: true, force: true });
    }
  });
