...
suite('Security review extension integration', () => {
  test('registers the command and defaults both review settings to enabled', async () => {
    const extension = vscode.extensions.getExtension('danisss9.angular-cli-plus'); assert.ok(extension); await extension.activate();
    assert.ok((await vscode.commands.getCommands(true)).includes('angular-cli-plus.reviewPackageSecurity'));
    const properties = extension.packageJSON.contributes.configuration.properties;
    assert.equal(properties['angularCliPlus.securityReview.afterInstall.enabled'].default, true);
    assert.equal(properties['angularCliPlus.securityReview.npmAudit.enabled'].default, true);
    assert.equal(extension.packageJSON.contributes.commands.find((command: { command: string }) => command.command === 'angular-cli-plus.reviewPackageSecurity').enablement, 'isWorkspaceTrusted');
  });
