...
function service(): SecurityReviewJobs {
  ...
  jobs = new SecurityReviewJobs(async (root, signal, installOutcome) => {
    ...
    return reviewPackageSecurity({ root, storage: context.globalStorageUri.fsPath,
      ...
      auditEnabled: vscode.workspace.getConfiguration('angularCliPlus', vscode.Uri.file(root)).get('securityReview.npmAudit.enabled', true),
...
export function endSecurityInstall(root: string | undefined, outcome: 'success' | 'failed' | undefined): void {
  if (!root) { return; }
  const enabled = vscode.workspace.isTrusted && vscode.workspace.getConfiguration('angularCliPlus', vscode.Uri.file(root)).get('securityReview.afterInstall.enabled', true);
