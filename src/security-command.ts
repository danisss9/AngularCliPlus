...
function service(): SecurityReviewJobs {
  ...
  jobs = new SecurityReviewJobs(
    async (root, signal, installOutcome) => {
      dismissed.delete(root);
      const context = getExtensionContext();
      return reviewPackageSecurity({
        root,
        storage: context.globalStorageUri.fsPath,
        rules: vscode.Uri.joinPath(
        ...
        signal,
        installOutcome,
        auditEnabled: vscode.workspace
          .getConfiguration('angularCliPlus', vscode.Uri.file(root))
          .get('securityReview.npmAudit.enabled', true),
        onProgress: (message) => {
          progress.set(root, message);
          void panels.get(root)?.panel.webview.postMessage({ type: 'progress', message });
        },
      });
    },
    (root, report, mode) => {
      reports.set(root, report);
      progress.delete(root);
      if (dismissed.has(root)) {
        return;
      }
      if (mode === 'manual' || report.findings.length || report.state !== 'complete') {
        const panel = openReport(root);
        void panel.panel.webview.postMessage({ type: 'report', report });
      } else {
        void panels.get(root)?.panel.webview.postMessage({ type: 'report', report });
        void vscode.window
          .showInformationMessage(
            'Package security review finished: no findings detected within the scanned scope.',
            'View Report',
          )
          .then((action) => {
    ...
    (root, error) => {
      progress.delete(root);
      logDiagnostic(`Package security review failed: ${error}`);
      void vscode.window.showErrorMessage(
        `Package security review failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    },
  );
...
export async function reviewPackageSecurityCommand(): Promise<void> {
  if (!vscode.workspace.isTrusted) {
    vscode.window.showErrorMessage('Package security review requires a trusted workspace.');
    return;
  }
  const folders =
    vscode.workspace.workspaceFolders?.filter((folder) => folder.uri.scheme === 'file') ?? [];
  if (!folders.length) {
    vscode.window.showErrorMessage('Open a filesystem workspace to review installed packages.');
    return;
  }
  const folder =
    folders.length === 1
      ...
      : await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select the workspace to review',
        });
  if (!folder || folder.uri.scheme !== 'file') {
    return;
  }
...
export async function reviewPackageSecurityForRoot(workspaceRoot: string): Promise<void> {
  ...
  const root = await validRoot(workspaceRoot);
  if (!root) {
    return;
  }
  openReport(root);
  service().request(root, 'manual');
}
export async function beginSecurityInstall(root: string): Promise<string | undefined> {
  const real = await validRoot(root);
  if (!real) {
    return;
  }
  service().beginInstall(real);
  reports.delete(real);
  progress.set(real, 'Waiting for installation to finish…');
  void panels
    .get(real)
    ?.panel.webview.postMessage({ type: 'progress', message: progress.get(real) });
  return real;
}
export function endSecurityInstall(
  ...
  const enabled =
    ...
    vscode.workspace
      .getConfiguration('angularCliPlus', vscode.Uri.file(root))
      .get('securityReview.afterInstall.enabled', false);
  ...
  if (!service().busy(root)) {
    ...
    void panels
      ...
      ?.panel.webview.postMessage({
        type: 'idle',
        message: 'Installation finished. Run Rescan to review the current installed files.',
      });
  }
}
...
function openReport(root: string): AnalysisPanel {
  ...
  const view = createAnalysisPanel('packageSecurityReview', 'Package Security Review', {
  ...
  view.onMessage<unknown>(async (raw) => {
    ...
    switch (message.command) {
      case 'ready':
        if (progress.has(root)) {
          await view.panel.webview.postMessage({ type: 'progress', message: progress.get(root) });
        } else if (report) {
          await view.panel.webview.postMessage({ type: 'report', report });
        }
        break;
      case 'rescan':
        if (vscode.workspace.isTrusted) {
      ...
      case 'cancel':
        ...
        break;
      case 'save': {
        if (!report || service().busy(root)) {
          return;
        }
        const destination = await vscode.window.showSaveDialog({
          defaultUri: vscode.Uri.file(path.join(root, 'package-security-report.html')),
        ...
        const [script, style] = await Promise.all(
          ['security-webview.js', 'security-webview.css'].map((file) =>
            fs.readFile(vscode.Uri.joinPath(assets, file).fsPath, 'utf8'),
          ),
        );
        const html = buildSecurityHtml({
      ...
      case 'openFile': {
        if (
          ...
          typeof message.evidenceIndex !== 'number'
        ) {
          return;
        }
        const target = await resolveEvidenceFile(
          root,
          report,
          message.findingId,
          message.evidenceIndex,
        );
        if (target) {
      ...
      case 'openReference': {
        if (
          !report ||
          typeof message.findingId !== 'string' ||
          !Number.isSafeInteger(message.referenceIndex) ||
          Number(message.referenceIndex) < 0
        ) {
          return;
        }
        const url = report.findings.find((finding) => finding.id === message.findingId)?.references[
          Number(message.referenceIndex)
        ];
  ...
  view.setHtml(
    buildSecurityHtml({
      nonce: randomBytes(24).toString('base64'),
      cspSource: view.panel.webview.cspSource,
      script: view.panel.webview
        .asWebviewUri(vscode.Uri.joinPath(assets, 'security-webview.js'))
        .toString(),
      style: view.panel.webview
        .asWebviewUri(vscode.Uri.joinPath(assets, 'security-webview.css'))
        .toString(),
    }),
  );
  return view;
